...
const serverFetcher = async ({
  ...
  const res = await fetch(
    `${SERVER_BASE_URL}/api/nft/contract/${contractAddress}/${tokenId}/transfers?chain=${chain}&page=${page}&page_size=${pageSize}`
  );
    ...
    useEffect(async () => {
      if (contractAddress && tokenId && !networkIsLoading && networkData) {
        ...
        try {
          ...
          console.error(err);
          setError("There was an error fetching contract token transfers");
          setData(null);
        }
        setIsLoading(false);
      }
